        ...
        <div className="flex gap-6 items-center">
          {socialLinks.map((link) => (
            <a 
              ...
              className="text-black hover:text-slate-600 hover:scale-125 transition duration-300 font-bold md:text-lg tracking-wide group relative"
              title={link.name}
            >
              <div className="flex items-center justify-center w-8 h-8 rounded-full border-2 border-black hover:border-slate-600 transition duration-300">
                {link.name === 'LinkedIn' && (
              ...
              <span className="hidden md:inline text-xs uppercase ml-1">{link.name}</span>
            </a>
          ))}
        </div>
      </div>
